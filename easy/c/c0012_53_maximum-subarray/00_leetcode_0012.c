...
int maxSubArray(int* nums, int numsSize) {
	...
	fprintf(stderr, "  maxSumSoFar=%d, maxISoFar=%d, maxJSoFar=%d \n", maxSumSoFar, maxISoFar, maxJSoFar);
	dump_array_range(nums, maxISoFar, maxJSoFar);
...
*/
	...
	int nums2[] = {-2,8,-3,4,-1,2,1,-5,4};
	numsSize = sizeof(nums2)/sizeof(nums2[0]);

	sum = maxSubArray(nums2, numsSize);
	fprintf(stderr, "sum=%d \n\n",	sum);
