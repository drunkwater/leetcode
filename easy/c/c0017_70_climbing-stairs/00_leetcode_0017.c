...
static uint64_t dfs(uint64_t n, uint64_t *count)
{
    if (n == 0) {
        return 0;
    } else if (count[n] > 0) {
...
static uint64_t climbStairs2(uint64_t n)
{
    uint64_t *count = malloc((n + 1) * sizeof(uint64_t));
    memset(count, 0, (n + 1) * sizeof(uint64_t));
    count[1] = 1;
    count[2] = 2;
    return dfs(n, count);
}
...
uint64_t climbStairs(uint64_t n) {
	/* sanity check */
...
int main( int argc, char *argv[] )
{
	...
	for (n = 0; n < 1024; n++)
	{
		...
		{
			fprintf(stderr, "FAILED! n=%d, a=%d, b=%d \n", n, a, b);
			break;
		}
	}
